...
Given any finite set $S$, the map $\vartheta_{\xi}$ defines a map $\vartheta_{\xi,S}\colon \Lm_{r'}(S)\to \Rm(S)$ by extending $\vartheta_{\xi}$ componentwise. For the purpose of this project, the special case where $\xi=1/2$ is enough, and we write $\theta_S$ from now on to denote $\vartheta_{\xi,S}$. 
