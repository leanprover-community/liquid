...
In this appendix, we gather a few results about homological algebra with normed abelian groups, the proofs of which are just obtained by keeping track of constants in the standard proofs. The argument we use, originated by a discussion with the author of ~\cite{Analytic}, is a modification of the original one. We need the following definition.

\begin{definition}
  \label{is_weak_bdd_exact_for_bdd_degree_above_idx}
  \lean{system_of_complexes.is_weak_bdd_exact_for_bdd_degree_above_idx}
  \leanok
  \uses{system_of_complexes}
  Let $(C_c^\bullet)_c$ be a system of complexes.
  For integers $m\geq 0$ and constants $k>0$, $c_0'>0$,
  the datum $(C_c^\bullet)_c$ is
  weakly $\leq k$-exact in degrees $\leq m$ and for $c\geq c_0'$ if the following condition is satisfied.
...
\begin{lemma}
  ...
  \uses{is_weak_bdd_exact_for_bdd_degree_above_idx, admissible}
...
\begin{lemma}
  ...
  \uses{is_weak_bdd_exact_for_bdd_degree_above_idx, is_bdd_exact_for_bdd_degree_above_idx, admissible}
  Let $M^\bullet_c$ be an admissible collection
...
\]
with $||m''^{i+1}_{k^3c}||_{M'^{i+1}_{k^3c}}\leq C + \epsilon_1$, where we choose $\epsilon_1 = \frac{\epsilon}{k ^ 3 + 2 * k + 1}$.
