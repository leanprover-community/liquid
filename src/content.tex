...
We first note that the difference between those two definitions is only about closed elements if we are ready to loose
...
\begin{proof}
  ...
  Lemma~\ref{is_bdd_exact_for_bdd_degree_above_idx_of_weakly} ensures we only need to care about closed elements
  of $M$. More precisely, let $x$ be a closed element in $M^i_{k^2c}$ for some $i ≤ m$ and $c ≥ c_0$.
