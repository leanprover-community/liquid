...
\begin{proof}[Proof of Theorem~\ref{explicit}]
  ...
  Fix a sequence $(c'_i)_i$ of nonnegative reals that is adept to $(\BD, c)$.
  (Such a sequence exists by Lemma~\ref{exists_adept}.)
  Moreover, we let $k'$ be the supremum of $k_0$ and the $c_i'$ for $i=0,\ldots,m$.
...
Finally, to check the third condition,
we use Definition~\ref{BD_h_mul} and Lemma~\ref{BD_h_mul_suitable} to find,
...
\]
By Lemma~\ref{homotopyNelements}, we can find this homotopy between the complex for $k'c$ and the complex for $c$, by our choice of $k'\geq c_i'$ for $i=0,\ldots,m$.
As $N$ is fixed, the homotopy is the universal homotopy from Lemma~\ref{homotopyNelements},
and in particular its norm is bounded by some universal constant $H$.
