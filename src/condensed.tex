...
\begin{remark}
  ...
  (They have been formalized in Lean though.)
...
\begin{definition}
  \label{strongly-exact}
  \lean{CompHausFiltPseuNormGrp₁.strongly_exact}
  \leanok
  \uses{png-cats}
  ...
  Let $r \colon \mathbb{R}_{\ge0} \to \mathbb{R}_{\ge0}$ be a function satisfying $c \le r(c)$ for all~$c$.
  We say that the sequence is \emph{strongly exact with respect to $r$} if
  $\mathrm{ker}(g)\cap X_{\leq c}\subset f(X'_{\leq r(c)})$.
\end{definition}

\begin{proposition}
  \label{strongly-exact-lim}
  \lean{CompHausFiltPseuNormGrp₁.strongly_exact.extend}
  \leanok
  \uses{bounded-limits, strongly-exact}
  Consider an inverse system
\[ (X_i'\overset{f}{\longrightarrow} X_i\overset{g}{\longrightarrow} X_i'')_i \]
  of exact sequences that are strongly exact with respect to~$r$ (independent of $i$).
...
\begin{proof}
  \leanok
  Pass to cofiltered limits of compact Hausdorff spaces in the statements $\mathrm{ker}(g)\cap X_{\leq c}\subset f(X'_{\leq r(c)})$,
...
\begin{definition}
  ...
  \end{align*}
  where $\underline{M}(S)$ is defined to be collection of functions $f \colon S \to M$ that factor as a continuous function through $M_c$, for some $c$.
  In symbols:
...
\begin{proposition}
  ...
  \lean{Condensed.exact_of_strongly_exact}
  \leanok
  \uses{CHPNG-to-Cond, strongly-exact}
  Consider an exact sequence of abelian groups
  \[ X'\overset{f}{\longrightarrow} X\overset{g}{\longrightarrow} X'' \]
  such that all of $X'$, $X$ and $X''$ carry the structure of compact-Hausdorffly-filtered-pseudonormed abelian groups.
  Assume that $f(X'_{\leq c})\subset X_{\leq c}$ and $g(X_{\leq c})\subset X''_{\leq c}$.
  If the sequence is strongly exact with respect to~$r$,
  then the sequence
...
\begin{proof}
  ...
  Since the sequence is exact with respect to~$r$,
  we know that for all~$c$ the natural map
  \[
    \phi \colon X_{\le c} \times_{X_{\le r(c)}} X'_{\le r(c)} \to X_{\le c} \times_{X''_c} \{*\}
  \]
  is surjective.
...
\begin{lemma}
  ...
  \leanok
  \uses{laurent-measures, real-measures, theta-exact_finite, CHPNG-Cond-exact, strongly-exact-lim}
...
\begin{proof}
  \uses{theta-exact_finite, CHPNG-Cond-exact, strongly-exact-lim}
  \leanok
  ...
  and by Proposition~\ref{strongly-exact-lim} we may also assume that $S$ is finite. With this reductions, the lemma is precisely Theorem~\ref{theta-exact_finite}.
\end{proof}
