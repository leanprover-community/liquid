...
\begin{definition}
  \label{exact-with-constants}
  \lean{condensed.exact_with_constants}
  \leanok
  \uses{png-cats}
  ...
  We say that the sequence is \emph{exact with constants $c_f$ and $c_g$} if
  $\mathrm{ker}(g)\cap X_{\leq c}\subset f(X'_{\leq c_fc})$ and $X''_{\leq c}\subset g(X_{\leq c_gc})$.
\end{definition}

\begin{proposition}
  \label{exact-with-constants-lim}
  \lean{CompHausFiltPseuNormGrp₁.exact_with_constants_extend}
  \leanok
  \uses{bounded-limits, exact-with-constants}
  Consider an inverse system
  \[ (0\to X'_i\xrightarrow{f_i} X_i\xrightarrow{g_i} X_i''\to 0)_i \]
  of short exact sequences that are exact with constants $c_f$ and $c_g$ (independent of $i$).
  ...
  Then
  \[ 0\to X'\xrightarrow{f} X\xrightarrow{g} X''\to 0 \]
  is exact with the same constants $c_f$ and $c_g$.
\end{proposition}
...
\begin{proposition}
  ...
  \lean{condensed.exact_of_exact_with_constants}
  \leanok
  \uses{CHPNG-to-Cond, exact-with-constants}
  Consider a short exact sequence of abelian groups
  \[ 0\to X'\xrightarrow{f} X\xrightarrow{g} X''\to 0 \]
  such that all of $X'$, $X$ and $X''$ carry the structure of compact-Hausdorffly-filtered-pseudonormed abelian groups.
  Assume that $f(X'_{\leq c})\subset X_{\leq c}$ and $g(X_{\leq c})\subset X''_{\leq c}$.
  If the sequence is exact with constants $c_f$ and $c_g$,
  then the sequence
...
\begin{proof}
  \uses{Ext-Lbar-aux, Qprime-prop, normed-to-cond}
...
\begin{proof}
  \uses{acyclic-sheaf, Ext-Lbar, Lbar-ses}
...
\begin{proof}
  \uses{theta.surjective, CHPNG-Cond-exact, exact-with-constants-lim}
  TODO
\end{proof}
