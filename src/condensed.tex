...
\begin{lemma}
  ...
  In other words,
  \[ \Ext^i_{\mathbb Z}^i(\Lbar_{r'}(S), V) \xrightarrow{[T⁻¹]_L - [T⁻¹]_V} \Ext^i_{\mathbb Z}^i(\Lbar_{r'}(S), V) \]
...
\begin{lemma}
  ...
  In other words,
  \[ \Ext^i_{\mathbb Z}^i(\Lm_{r'}(S), V) \xrightarrow{[T⁻¹]_L - [T⁻¹]_V} \Ext^i_{\mathbb Z}^i(\Lm_{r'}(S), V) \]
...
\begin{proof}
  ...
  And by Lemma~\ref{Ext-Lbar} all groups $\Ext_{\CondMod_{\mathbb Z[T^{-1}]}}^i(\Lbar{r'}}(S), V)$ vanish for $i \ge 0$.
  The result follows.
...
\begin{proof}
  ...
  Now use Lemma{Ext-L} to obtain isomorphisms between the Ext-groups involving $\Lm_{r'}(S)$, for $i > 0$,
