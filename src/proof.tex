...
is a complex, and for all $\widehat{y}\in \ker \widehat{d}_1$ and all $\epsilon>0$
...
\begin{lemma}\label{lem:controlled_exactness}
  ...
  \begin{itemize}
    \item
      for every $m$ in $\ker g$, there exists $m' ∈ M_0$ such that
      $f(m') = m$ and $\|m'\| ≤ C\|m\|$.
    \item
      for every $x$ in the image of $g$, there exists $y ∈ M_1$ such that
      $g(y) = x$ and $\|y\| ≤ D\|x\|$.
  \end{itemize}
  Then for every $\hat m$ in $\ker \hat g$ and every positive $ε$,
  there is some $\hat m'$ in $\hat M_1$ such that
  $\hat f(\hat m') = \hat m$ and $\|\hat m'\| ≤ (C + ε)\|\hat m\|$.
\end{lemma}

\begin{proof}\leanok
  First, we claim that $\ker(g : M_1\to M_2)$ is dense in
  $\ker(\widehat{g} : \widehat{M}_1\to \widehat{M}_2)$.
  ...
  We now build the announced preimage for any $\hat m$ in $\ker \hat g$.
  First note the conclusion is trivial when $\hat m = 0$, so we can assume
  $\hat m ≠ 0$.
  Using the density result proven above, one can write $\widehat{m}$ as a sum
  $\sum_{i \ge 0} m_i$ with all $m_i\in \ker(g)$, $\|\hat m - m_0\| ≤ ε_0$ and
  $‖m_i‖\leq \epsilon_i$ for $i>0$ for any given sequence of positive numbers
  $\epsilon_i$. We can then lift each $m_i$ to $m'_i$ such that
  $f(m'_i) = m_i$ and $‖m'_i‖\leq C‖m_i‖$, and then set
  $\widehat{m}'=\sum m'_i$. Because $\widehat{M}_0$ is complete,
  this sum converges provided the $ε_i$ sequence converges fast enough to zero.
  We then have $\hat f(\widehat{m}') = \widehat{m}$ and
  \[
    ‖\widehat{m}'‖ ≤ C\sum_{i \geq 0} ‖m'_i‖ ≤
    C(\|\hat m\| + ε_0) + C\sum_{i>0} ε_i ≤
    (C+\epsilon)‖\widehat{m}‖
  \]
  where the last inequality holds provided the $ε_i$ sequence converges fast
  enough to zero (this is where we use that $\widehat{m} ≠ 0$).
\end{proof}
