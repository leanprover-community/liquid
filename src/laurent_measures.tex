...
For simplicity, denote by $\Z(\!(T)\!)_{r'}$ the group $\Lm_{r'}(\ast)$. 
...
One of the key results proven in \cite[\S 6]{Analytic} is the surjectivity and continuity of the map $\vartheta_\xi$, together with the proof that its kernel is principal for every $0<\xi<1$. To state this result, we begin with the following lemma.
...
\begin{lemma}
	...
	\leanok
%Let $0<p'<1$ be a real number, let $S$ be a finite set, and let $r'$ denote $\left(\frac{1}{2}\right)^{p'}$. 
...
\begin{theorem}
  ...
  \leanok
%  \lean{ses.continuous_θ_c}
Let $0<p'<1$ be a real number, let $S$ be a profinite set, and let $r'$ denote $\left(\frac{1}{2}\right)^{p'}$. The sequence
\[
